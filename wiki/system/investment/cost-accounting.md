...
> **Scope:** si conservano commissioni e consumo/token reali come dati di audit, utili a stimare costi futuri. Stima preventiva fine, net-EV e metering completo sono **Post-MVP**; questa pagina è una spec, non stato del repository esterno.
...
- **Transaction cost auto-adattivo**: nessun costo hardcodato; si legge la commissione reale del momento (broker/asset/size). → [[system/foundation/decision-log]] (2026-06-02).
- **Token cost = commissione**: il costo dei token OpenRouter, convertito in $/€, è trattato come una commissione del broker e sottratto dal profitto atteso. → [[system/agents/agents]].

## Proposta: gestione in tre momenti

### 1. Pre-trade (stima) — entra come guardrail
- Ogni broker espone un `CommissionModel.estimate(order) -> cost` che legge la fee reale del momento (broker · asset · size · spread). Vive nell'**adapter broker** ([[system/execution/execution]]).
...
- La **net performance reale** alimenta la rendicontazione (portfolio_state) e il [[system/quant/learning-feedback-loop]] (un'alpha che non copre i costi va smascherata presto).
...
- **Budget per ciclo** (opzionale): un tetto di token/costo per ciclo come freno anti-overtrading; si lega ai criteri "info sufficienti" del PM ([[system/orchestration/parallelism-design]]).

## Dove vive (riassunto)
| Pezzo | Dove |
|-------|------|
| `CommissionModel.estimate` | adapter broker (`tradingagents/broker/`) |
| net-EV / R:R-after-cost check | funzione Trade / Risk gate ([[system/execution/execution]]) |
| campi costo (`commission`, `token_cost`) | storage (`trades` o tabella `costs`) |
| token meter | wrapper LLM / client OpenRouter |

## Stato
> 🟢 **Prima implementazione (alpha v0, 2026-06-06)** — `tradingagents/broker/commission.py` (`CommissionModel`: Zero/PerTrade/PerShare/Percent) + `tradingagents/execution/costs.py` (`expected_reward`/`expected_risk`, `assess_costs` = **guardrail net-EV**); integrato nel `run_cycle` ([[system/orchestration/trigger-engine]]): prima di eseguire, se la ricompensa potenziale non copre commissione + token cost → **no-trade** (`skipped_cost`). **Mancano ancora**: consuntivo post-fill (campi `commission`/`token_cost` sul `trade`), token metering reale da OpenRouter, fee auto-adattiva dal broker live.

Resto **aperto / da progettare**. Card in [[artifacts/project-board]]; decisione in [[system/foundation/decision-log]].
