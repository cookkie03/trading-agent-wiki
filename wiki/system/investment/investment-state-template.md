...
>
> Questo NON è lo schema definitivo: lo schema "vivo" e già ragionato è in [[system/investment/state-schemas]]. Qui c'è il **catalogo esteso** da cui quello si raffina.
...
> Promemoria: il modello corrente usa uno **stato annidato a campi fissi**: le sezioni di ricerca vengono progressivamente riempite, validate in modo collettivo e poi sintetizzate dal PM. Qui le trattiamo come un unico catalogo e marchiamo *quando* ogni campo si compila. Vedi naming in [[system/investment/state-schemas]].
...
*(chi scrive: tool di iniezione stato portafoglio → [[system/agents/agents]])*
...
## C — Analisi dei desk
*(chi scrive: i due desk analisti → [[system/agents/agents]])*
...
| `key_factors` | list[obj] | 🟢 CORE | fattori rilevanti + valore + come letti → [[system/quant/quant-backtesting]] |
...
| `conviction_level` | **enum** (5 livelli) | 🟢 CORE | assegnato dal PM → [[system/investment/rating-scoring]]. **Deciso 2026-06-04: enum, non score 0-100** |
...
| `k_entry` | float (ATR) | 🟢 CORE | coefficiente di sconto d'entrata (scalato per conviction) → [[system/investment/state-schemas]] |
...
| `position_sizing` | float % | 🟢 CORE | % del portafoglio, mai assoluto → [[system/investment/position-sizing]] |
...
| `linked_trades` | list | 🔵 | eventuali vendite coordinate per far spazio (batch) → [[system/investment/rating-scoring]] |

---

## E — Gate di rischio
*(chi scrive: Risk Analyst → [[system/agents/agents]])*
...
| `agent_contributions` | obj | 🔵 | traccia di chi ha scritto cosa → base per lo scoring agenti ([[system/investment/rating-scoring]] §2) |
| `past_context` | str | 🟢 CORE | lezioni dai trade precedenti sullo stesso ticker, segmentate per `exit_reason` → [[system/investment/rating-scoring]] §4 |
| `cost_estimate` | obj | 🟡 | token cost del ciclo + commissioni stimate (net performance) |
| `chain_of_thought_ref` | ref | 🔵 | puntatore al log del ragionamento (learning loop) → [[system/quant/learning-feedback-loop]] |
...
5. **Un solo state ricco o sub-state annidati?** (es. blocco `risk` separato) — più una scelta di engineering del grafo → [[system/investment/state-schemas]].
6. **Campi per il futuro multi-asset** (opzioni, commodity): predisporli ora "vuoti" o aggiungerli dopo?

---

*Pagina-compagna di [[system/investment/state-schemas]] (schema vivo). Da consumare in una sessione dedicata con Salvatore; ogni decisione presa torna a raffinare lo schema vivo e migra nel [[system/foundation/decision-log]].*
