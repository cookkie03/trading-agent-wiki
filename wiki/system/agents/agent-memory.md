...
> **Scope:** memoria intra-task e memoria semantica sono requisiti documentali. L'interesse per RAG + grafi diventa una ricerca Post-MVP, dopo aver stabilizzato dati, deduplicazione e contratti di recupero.

> Due livelli distinti. L'**intra-task** è un requisito del design corrente; l'**inter-task** è un tema di design **da affrontare successivamente** (input di Luca 2026-06-07): come ottimizzare la memoria persistente degli agenti *tra* i task, per imparare dai casi/errori precedenti. Il vault non attesta l'implementazione di nessuno dei due nel repository esterno.
...
5. **Pesi/scoring degli agenti dal backtest/esiti** — la hit-rate per-agente come *memoria di affidabilità* in input al PM ([[system/quant/learning-feedback-loop]] §4).
6. **Altre** — da discutere insieme.

### Note di design
- Collegare al [[system/quant/learning-feedback-loop]]: la memoria inter-task È il loop di apprendimento operativizzato lato agenti.
...
## Stato
**Aperto / da discutere.** Card in [[artifacts/project-board]]; decisione aperta in [[system/foundation/decision-log]]. Prerequisito comodo: la **deduplicazione DB** (anch'essa da fare) per una memoria pulita.
