...
> **Stato:** questa è la specifica dei prompt e del loro metodo. Il framework scelto è **LangGraph + LangSmith**; i prompt v0 devono essere riletti con Salvatore prima di essere considerati pronti per un repository esterno.

> **Riferimenti da studiare, non codice del progetto:** il prompt engine di [[prior-art/libraries/rizzo-trading-agent]] e la [[prior-art/tradingagents/code-wiki|Code Wiki di TradingAgents]] sono fonti di pattern; ogni adozione passa comunque dalla presente specifica, dagli schemi e dalla revisione con Salvatore.
...
| **Schema di output** | *Forma* della risposta (campi, tipi) — JSON-strict | [[system/investment/state-schemas]] (Pydantic) |
| **Tool** | *Capacità*: cosa può chiamare | [[system/tools/tools-inventory]] |
...
7. **Criterio di stop esplicito** — la checklist di copertura ([[system/agents/agent-behaviors]]); l'agente sa quando ha finito, il PM può comunque richiamarlo.
...
> Istanze concrete dello scheletro, una per agente ([[system/agents/agent-behaviors]]). Sono **bozze v0** da rifinire iterativamente con LangSmith; l'impianto (struttura + contenuti) è approvato.

> **Revisione condivisa richiesta:** Luca e Salvatore leggono i prompt per validare ruoli, fonti, criteri di stop e linguaggio di mercato. La card relativa vive in [[artifacts/project-board]].
### Technical
