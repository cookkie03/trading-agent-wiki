...
> 🟢 **Esteso (2026-06-06)**: oltre ai prezzi, ora si ingeriscono **news** (`NewsItem` + `ingest_news`, DB-first dedup, `YFinanceNewsFetcher`) e **fondamentali** (`FundamentalSnapshot` + `ingest_fundamentals`, `YFinanceFundamentalsFetcher`). I desk **Market**/**Sentiment** leggono le news dal DB, il desk **Fondamentali** legge i metrics. Restano placeholder: **macro (FRED)** e **social-sentiment** (Reddit/StockTwits/X). Mancano ancora: queue + adaptive extractor (rate-limit), mantainer.
