...
2. **Live / Storico** — determina la **precedenza** (decisione 2026-06-05):
   ...
   - **Storico / immutabile** (barre passate, bilanci depositati) → **check-presenza DB-first** ([[system/data/data-layer]]); inutile ri-scaricare ciò che non cambia.
...
5. **Vendor** — fonte primaria / secondaria, dal mapping in [[system/data/data-providers]].

### Due regole trasversali (proposte come vincolanti)
- **Parametrici, mai hardcoded**: `get_ohlcv_history(ticker, start, end, interval)`, non "ultimi 30 giorni" (già nello stack, [[system/data/data-providers]]).
- **Adaptive extractor layer**: ogni tool live passa per guardrail di frequenza/rate-limit dell'adaptive extractor ([[system/data/data-layer]]).
...
---
> **Policy 2026-07-13:** l'inventario parte dalle capacità esposte da OpenBB, poi documenta provider broker e fallback. La tabella provider dettagliata è [[system/data/data-vendors]]; questa pagina resta l'interfaccia stabile vista da agenti e moduli.
...
> L'ATR usato dal sizing/`entry_price` arriva da qui ([[system/investment/position-sizing]]). Indicatori "caldi" possono essere **pre-calcolati** in materialized view ([[system/data/db-access-performance]]); il tool resta per richieste parametriche fuori dal set pre-calcolato.
...
| `compute_fundamental_ratio(ticker, ratio, params)` — ratio derivati da dati e ratio semplici | calcolo su dati normalizzati | cacheabile | Fondamentali | interno |
...
| Tool                                                           | Live/Storico | Write-through | Agente                                   | Vendor  |
| -------------------------------------------------------------- | ------------ | ------------- | ---------------------------------------- | ------- |
| `get_calendar(type, window)` — earnings + calendario economico | semi-live    | ✅             | Market, PM (alimenta i trigger di alert) | OpenBB / provider dedicati / broker, da valutare |
...
> `inject_portfolio_state` è il tool deciso da Luca 2026-06-04 ([[system/agents/agents]]); `get_open_positions_risk` serve al cap di portfolio heat del [[system/investment/position-sizing]].

> I check di portafoglio sono **periodici e costanti**, anche quando l'analisi è event-driven; vedi [[system/orchestration/trigger-engine]].
### H — Opzioni (leva)
| Tool | Live/Storico | Write-through | Agente | Vendor |
|------|--------------|---------------|--------|--------|
| `get_options_chain(ticker, expiry, type)` — quote Call/Put per la leva | **live** | ✅ | Risk, PM | IBKR · Tradier |

> Attivo solo in fase `Strong Buy`/`Strong Sell` validata ([[system/agents/agents]] §Leva). Fuori MVP (Alpaca non copre opzioni reali).

### I — Guardrail Statuto (NON sono tool LLM)
I controlli dello Statuto (max % per area, VaR di portafoglio, diversificazione, riserva 10% cash) **non** sono tool che l'agente chiama: sono **check Python deterministici** nel gate del Risk Analyst ([[system/agents/agents]] §Guardrail). Elencati qui solo per completezza, per evitare di trasformarli per errore in tool LLM.
...
**Ancora aperti:**
3. **Vendor dei dati live** (famiglie A/D/F — news, quote, calendario): la ricerca e le schede di copertura vivono in [[system/data/data-vendors]]; il catalogo interrogabile è [[artifacts/tool-catalog.base]].
...
Una volta fissato l'inventario, il livello successivo è il **comportamento per-agente**: per ciascun desk, *quali* di questi tool usa, in che ordine, con quale criterio di stop → [[system/agents/agents]] (TODO aperto).
