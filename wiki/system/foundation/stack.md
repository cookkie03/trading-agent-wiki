...
Scelte tecnologiche e requisiti del progetto. Gli snapshot di repository sono reference design: per la policy su ciò che la wiki può attestare, vedi [[system/foundation/implementation-status]].
...
| Schedulazione | **Event-driven + health check periodico** | Alert, calendario e `next_check_date` avviano l'analisi; il controllo di portafoglio/sistema resta costante e separato |
...
| Astrazione broker | **Adapter per broker** | Un wrapper per broker che traduce l'API in interfaccia interna standard → broker intercambiabili come i provider LLM (2026-06-02). Vedi [[system/execution/execution]] |
...
| PM decisore | **Claude di frontiera** | Preferenza espressa da Luca per la decisione/aggregazione finale; modello da fissare esternamente alla wiki |
| Desk analisti | **DeepSeek via OpenRouter** | Preferenza corrente per costo/qualità nelle analisi specialistiche |
...
| Componente | Scelta | Motivazione |
|------------|--------|-------------|
| Framework agenti | **LangGraph** | Scelta chiusa nella rilettura 2026-07-13; [[prior-art/libraries/datapizza-ai]] resta reference storica |
| Workflow / grafi | **LangGraph StateGraph** | Grafo esplicito, state strutturato e subgraph per ticker |
| LLM router | **OpenRouter** | Indipendente dal framework di orchestrazione |
| Debug/Evaluation | **LangSmith** | Osservabilità e valutazione del workflow |
| Verifica grafi | **Mermaid / diagrammi LangGraph** | Confronto tra grafo e design della wiki |

**Contesto storico (2026-06-17)**: una build precedente usava Datapizza AI. Il blocco seguente è preservato solo come reference design e non descrive né prescrive il repository esterno.
