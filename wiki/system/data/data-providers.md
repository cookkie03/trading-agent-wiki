...
Panoramica storica di broker e provider. La policy operativa aggiornata è [[system/data/data-vendors]]: OpenBB guida il catalogo di copertura, broker e altri provider restano fonti intercambiabili dietro wrapper interni.

> **Distinzione fondamentale**: il broker fornisce esecuzione, stato conto e, se utile, quote real-time via WebSocket. La popolazione dati usa capacità e provider separati. Ogni record deve mantenere provenienza, date e politica di retention.
...
- **Darwinex** — progettata per algo trader: collega Python → MT5 → Darwinex → DARWIN (prodotto finanziario verificato FCA). Compenso: 20% performance fee. Integrazione diretta con IBKR. Da attivare dopo il paper trading. → [[system/foundation/ideas-log]]
