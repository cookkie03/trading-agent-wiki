...
> Questa pagina distingue requisito e ottimizzazione: DB-first, output strutturati e provenienza dati sono requisiti; cache distribuita, tuning avanzato e scelte di modelli leggeri per controlli frequenti restano valutazioni successive. LangGraph + LangSmith è il framework documentato in [[system/foundation/stack]].

> Approfondimento sul **come** il DB centrale viene letto/scritto e su come renderlo veloce e poco interrogato. Il *cosa* contiene (aree logiche, extraction, retention) sta in [[system/data/data-layer]]; qui sta l'ingegneria di accesso.
...
- **Owner**: modulo [[system/data/data-layer]] (🛠 Luca).
...
2. **Snapshot di ciclo in memoria** *(= tool di iniezione del portafoglio)*: all'avvio del ciclo del PM si carica **una volta** `portfolio_state` + context, lo si mette nello `state` del grafo LangGraph e **tutti i nodi leggono da lì**, non dal DB. Vedi [[system/agents/agents]].
...
- 🛠 **Graceful shutdown & recovery**: già aperta in [[system/data/data-layer]] — la strategia di ripresa dal checkpoint resta da definire.

---

*Vedi [[system/data/data-layer]] per le aree del DB e l'extraction, [[system/foundation/decision-log]] per lo storico decisioni.*
