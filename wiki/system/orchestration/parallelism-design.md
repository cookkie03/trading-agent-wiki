...
> **Spec di design:** subgraph isolati per ticker, concorrenza limitata e ordini seriali. I dettagli di thread pool, DB e branch precedenti sono reference design; la forma concreta non è attestata dal vault. Vedi [[system/orchestration/universe-watchlist]].
...
```
TRIGGER (alert · calendario · next_check_date · health check)
   │
   ▼
...
**Staleness di B — risolta da cose già decise**: ogni fatto nella scheda ha `publication_date`/`reference_date` + confidence; il mantainer marca stale ciò che è vecchio/contraddetto; e gli agenti fanno **real-time-first** sui dati decision-critical → la scheda è una *base*, non un *oracolo* ([[system/agents/agents]]).
...
Il PM orchestratore deve poter valutare N ticker (es. tutto l'universo S&P investibile). Ogni ticker ha il suo `research_state` ([[system/investment/state-schemas]]). Vincoli:
...
- **rate limit**: gli extractor non possono essere chiamati in parallelo senza coordinamento (vedi [[system/data/data-layer]]);
...
| **Usa le info passate?** | **Sì**: segnali quant già nel DB (momentum, volatilità/ATR, distanza da 52w high/low, variazione %, z-score volumi, eventuali ratio fondamentali) + pesi dal **feedback storico** del [[system/quant/learning-feedback-loop]]. Produce un **ranking grezzo, non una tesi**. |
| **Chi lo aggiorna?** | **Nessun attore nuovo**: gli **extractor** portano i dati freschi, il **mantainer/quant** ricalcola gli score. È un **job deterministico periodico** (+ on-trigger). Lo screening è una *vista derivata* che si ricalcola all'arrivo di dati nuovi. → [[system/data/data-layer]]. |
...
- **Completezza dello state**: il gate di completezza ([[system/investment/state-schemas]]) è già un criterio minimo necessario.
...
Equilibrio con l'anti-loop: i tetti sopra (budget iterazioni, stabilità tesi) sono una **rete di sicurezza per non andare all'infinito**, *non* una scusa per decidere su informazione parziale. Ordine di priorità: **prima chiarisci il dubbio chiedendo ai desk**, e solo quando il dubbio è *davvero* irrisolvibile entro il budget → allora decidi con ciò che hai o **scegli il no-trade** (l'astensione è sempre un'opzione lecita e preferibile a un trade su basi incerte). → istruzione da formalizzare nel system prompt, vedi [[system/agents/agents]].

---

*Tracciato in [[artifacts/project-board]] come decisione da prendere con Salvatore. Vedi [[system/agents/agents]] per il ruolo del PM.*
