...
- indipendente dalla dimensione del capitale (scala da paper trading a reale);
...
Far dipendere la dimensione dal **`conviction_level`** dello state (Luca: *«l'idea di farlo dipendere da un conviction level è molto valida»*). Logica di massima:
...
- **Cap deterministici** applicati dopo: lo Statuto taglia qualsiasi size che violi VaR, % max per area/settore, riserva cash. Vedi guardrail in [[system/modules/agents]].

---

## Kelly Criterion (richiesto da Luca — spiegazione)

> Luca: *«cos'è il Kelly criterion? Aggiungilo alla wiki»*.
...
- Kelly puro è **molto aggressivo**: in pratica si usa **frazionario** (es. *half-Kelly*, metà del valore) perché stimare male `p` porta a sovraesposizione e drawdown pesanti.
...
3. **v2**: sizing volatility-adjusted (size inversamente proporzionale alla volatilità del titolo, es. via ATR — più volatile = posizione più piccola a parità di rischio).
