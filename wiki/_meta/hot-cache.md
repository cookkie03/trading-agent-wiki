...
- **Warm start + tool-calling coesistono (2026-06-07)**: state vuoto = trigger → `brain/warmup.py` pre-lancia gli extractor (senza chiamata dell'agente) e popola il **primo contesto iniettato**; durante l'analisi gli agenti fanno comunque tutto il tool-calling che serve. Entrambi i meccanismi presenti, come richiesto da Luca.
