...
- **Board rifatta e corretta dopo review**: [[artifacts/project-board]] resta una Kanban valida in Obsidian; le colonne sono ordinate per meeting, Salvatore, Luca, coding agent, ricerca, in corso e fatto. Le card restano sintetiche ma puntano alle pagine che governano davvero il dettaglio.
...
- **Pagine hub rese più operative**: [[system/codebase-architecture]] ora esplicita package boundary e slice di build; [[system/data-sources-tool-map]] collega provider → wrapper → tool; [[system/frontend-module]] definisce il boundary read-only/sostituibile; [[system/system-wiki-reorganization]] contiene mapping commenti → destinazioni e priorità di pulizia.
- **LightRAG**: tentativo effettuato su KB `trading-agent-wiki`, ma le query hanno restituito `no-context`; per questo run la fonte affidabile resta il vault locale letto direttamente.
...
- **Tool-calling autonomo FATTO (2026-06-07)**: gli agenti emettono le tool call da soli (`brain/llm.py` bind_tools loop); `build_desk_tools` dà a ogni agente tutti i tool utili (extract→risponde→write-through). **Mantainer** fatto. Allineamento canvas in [[system/canvas-code-mapping]].
- **Warm start + tool-calling coesistono (2026-06-07)**: state vuoto = trigger → `brain/warmup.py` pre-lancia gli extractor (senza chiamata dell'agente) e popola il **primo contesto iniettato**; durante l'analisi gli agenti fanno comunque tutto il tool-calling che serve.
