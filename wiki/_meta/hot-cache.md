...
- **Operazione principale**: **Step 1 — design `entry_price`**. Ripartenza da "riprendiamo", sessione condotta da Claude. Progettata e scritta in [[system/state-schemas]] la **proposta per `entry_price`**: backbone in **unità di ATR** (entry/stop/tp da `current_price ± k·ATR`, l'LLM dà i coefficienti `k_*`, Python traduce in prezzo), `k_entry` **scalato per conviction**, **guardrail R:R** deterministico (default 1.5). Ciclo di vita limit non colpito → scade alla `next_check_date`. Default prima alpha: ATR(14), `k_stop=2`, `k_tp=3`. **È una proposta da approvare** (Luca leggerà e darà sì/no). Aggiornati board (card entry_price → "proposta pronta"), log. Aperti residui dello state: granularità conviction · n. state annidati · storage state · dove avviene l'aggregazione.
